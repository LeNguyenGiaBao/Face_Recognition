...
file_install = os.getcwd() + '/facenet_pytorch/models/weights/'
...
class PNet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        self.prelu1 = nn.PReLU(10)						# PReLU(x)=max(0,x)+a∗min(0,x)
        ...
        if pretrained:
            ...
            state_dict_path = file_install + 'pnet.pt'
...
class RNet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            ...
            state_dict_path = file_install + 'rnet.pt'
            state_dict = torch.load(state_dict_path)
            self.load_state_dict(state_dict)
...
class ONet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            ...
            # state_dict_path = cwd + '/facenet_pytorch/data/onet.pt'
            # state_dict_path = os.path.join(file_install, '/data/onet.pt')
            state_dict_path = file_install + 'onet.pt'
            state_dict = torch.load(state_dict_path)
            self.load_state_dict(state_dict)
